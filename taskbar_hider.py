...
# GUI (Tkinter)
# ──────────────────────────────────────────────
class TaskbarHiderGUI:
    def __init__(self, core: TaskbarHiderCore):
        self.core = core
        self.root = tk.Tk()
    ...
    def _build_ui(self):
        ...
        ttk.Label(header, text="TaskbarHider", style="Title.TLabel").pack(side=tk.LEFT)
...
if __name__ == "__main__":
    ...
    if platform.system() != "Windows":
        ...
        print("  python taskbar_hider.py")
        print()
        print(".exe로 패키징하려면:")
        print("  pip install pyinstaller")
        print('  pyinstaller --onefile --windowed --name TaskbarHider taskbar_hider.py')
        sys.exit(1)

    core = TaskbarHiderCore()
    gui = TaskbarHiderGUI(core)
    gui.run()
